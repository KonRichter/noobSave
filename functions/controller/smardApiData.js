...
const getBrownCoalAndSetCommonTime = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Coal', freshData(data.series)]);
...
const getNuclear = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Nuclear', freshData(data.series)]);
};
...
const getWindOffshore = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Wind', freshData(data.series)]);
};
...
const getWater = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Hydro', freshData(data.series)]);
};
...
const getOtherConventionals = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Others', freshData(data.series)]);
};
...
const getBiomass = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Biomass', freshData(data.series)]);
};
...
const getPhotovoltaics = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['Solar', freshData(data.series)]);
};
...
const getNaturalGas = async (startTime, energyData) => {
  ...
  energyData.energySources.push(['NaturalGas', freshData(data.series)]);
};
...
const roundByLargestRemainder = (arr) => {
  ...
  return valuePartsArr
    .sort((x, y) => x.originalIndex - y.originalIndex)
    .map((p) => [p.energySource, p.mwhValue, p.integer]);
};
...
const calculateRoundedValues = (energyData) => {
  ...
  for (let i = 0; i < energyData.energySources.length; i++) {
    energyData.energySources[i].push(
      (energyData.energySources[i][1] / sumMWh) * 100
    );
